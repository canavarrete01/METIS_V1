...
def cylinder_to_cartesian(rho, theta, x):
...
def cylinder_to_sphere(rho, theta, x):
    # rho and x in same unit of length
    # theat in radians
...
# def sphere_to_cylinder(r, long, lat):
#     # r same unit of length
#     # long and lat in radians
#     x,y,z = sphere_to_cartesian(r,long,lat)
#     rho, theta, xx = cartesian_to_cylinder(x,y,z)
#     return np.array([rho, theta, xx])
    
# def sphere_to_cartesian(r,long,lat):
#     # r same unit of length
#     # long and lat in radians
#     x = r*np.sin(lat)*np.cos(long)  
#     y = r*np.sin(lat)*np.sin(long)
#     z = r*np.cos(lat)
...
# def cartesian_to_cylinder(x,y,z):
#     # x, y, z in same units of length
#     # rho and xx in same unit of length
#     # theat in radians    
#     theta = np.arctan2(y,x)  # arctan domain is - pi/2 to pi/2, arctan2 uses quadrants of y and x  to get right value 
#     theta = (theta + 2.0*np.pi)%(2.0*np.pi)    
#     rho = np.sqrt(x**2.0+y**2.0)
#     xx = z
#     return np.array([rho,theta,xx])

# -- For transit spectra integration --------------------    
def calc_path_lengths(alt, long, lat, rho, theta):
    '''
...
def compute_transmittance(alt, long, lat, Tmap, Pmap, OpacFunc, rho, theta, Z, wlrange):
    '''
...
def effective_area_at_full_depth_no_long_dep(alt, long, lat, Tmap, Pmap, OpacFunc, nwl, nrho, Z, wlrange):
    ''' 
